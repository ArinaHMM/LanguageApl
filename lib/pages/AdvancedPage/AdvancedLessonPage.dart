...
class AdvancedLessonPage extends StatefulWidget {
  ...
  @override
  _AdvancedLessonPageState createState() => _AdvancedLessonPageState();
}

class _AdvancedLessonPageState extends State<AdvancedLessonPage> {
  List<dynamic> words = [];
  int currentIndex = 0;
  ...
  Future<void> updateUserProgress(String lessonId, int newProgress) async {
    ...
    if (currentUser != null) {
      ...
      await userDoc.update({
        'progressadv.$lessonId': newProgress,
      });
    }
  }
  ...
  Future<void> _checkAndUpdateUserLevel() async {
    ...
    if (currentUser != null) {
      ...
      if (userSnapshot.exists) {
        ...
        final currentLevel = userData['language'] ?? 'Elementary';

        // Проверяем, что текущий уровень пользователя - Beginner
        if (currentLevel == 'Elementary') {
          // Получаем все уроки на уровне Beginner
          final beginnerLessonsSnapshot = await FirebaseFirestore.instance
              .collection('advancedlessons')
              .where('level', isEqualTo: 'Elementary')
              .get();

          // Проверяем, выполнены ли все уроки на уровне Beginner
          bool allLessonsCompleted = beginnerLessonsSnapshot.docs.every((doc) {
            final lessonProgress = userData['progress'][doc.id] ?? 0;
            return lessonProgress == 100;
          });

          // Если все уроки завершены, обновляем уровень пользователя на Intermediate
          if (allLessonsCompleted) {
            await userDoc.update({'language': 'Intermediate'});
            print('Уровень пользователя обновлен до Elementary');
          }
        }
      }
    }
