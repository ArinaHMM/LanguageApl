...
class MessagesPage extends StatefulWidget {
  ...
  final String? initialOtherUserName; // Имя собеседника для отображения в AppBar
...
class MessagesPageState extends State<MessagesPage> {
  ...
  final FirebaseAuth _auth = FirebaseAuth.instance; // Для получения текущего пользователя
  ...
  void initState() {
    ...
    if (widget.initialOtherUserName != null && widget.initialOtherUserName!.isNotEmpty) {
    ...
    try {
      ...
      if (chatDoc.exists && chatDoc.data() != null) {
        ...
        if (user1 != null && user2 != null) {
          ...
          if (widget.initialOtherUserName == null || widget.initialOtherUserName!.isEmpty) {
            // Если имя не было передано, загружаем его
            DocumentSnapshot<Map<String, dynamic>>? otherUserDoc = await _usersCollection.getUser(otherUserId!);
            if (mounted && otherUserDoc != null && otherUserDoc.exists) {
              final otherUserData = otherUserDoc.data();
              setState(() {
                _appBarTitle = otherUserData?['firstName'] as String? ?? otherUserData?['email'] as String? ?? 'Собеседник';
              });
            } else if (mounted) {
          ...
          print("Error: user1id or user2id missing in chat doc ${widget.chatId}");
  ...
  Future<void> _markMessagesAsReadByCurrentUser() async {
    ...
    UserModel? currentUserModel = await _usersCollection.getUserModel(currentUserUID!);
    bool isSupportAgent = currentUserModel?.role == UserRoles.support || currentUserModel?.role == UserRoles.admin;

    Map<String, dynamic> updateData = {};
    if (isSupportAgent) {
      updateData['isReadBySupport'] = true;
      updateData['unreadCountBySupport'] = 0;
    } else { // Это обычный пользователь (клиент)
      updateData['isReadByUser'] = true;
      updateData['unreadCountByUser'] = 0;
    }

    try {
      final chatDocRef = _firestore.collection('support_chats').doc(widget.chatId);
      final chatDoc = await chatDocRef.get();
      if (chatDoc.exists) {
        // Обновляем только если есть что обновлять (например, были непрочитанные)
        bool needsUpdate = false;
        if (isSupportAgent && (chatDoc.data()?['unreadCountBySupport'] ?? 0) > 0) needsUpdate = true;
        if (!isSupportAgent && (chatDoc.data()?['unreadCountByUser'] ?? 0) > 0) needsUpdate = true;
        
        if (needsUpdate) {
          await chatDocRef.update(updateData);
          print("Messages in chat ${widget.chatId} marked as read by $currentUserUID");
        }
      }
    } catch (e) {
  ...
  Future<void> sendMessage({String? imageUrl}) async {
    ...
    if ((text.isEmpty && imageUrl == null) || currentUserUID == null || widget.chatId.isEmpty) {
      return;
    }
    ...
    try {
      ...
      UserModel? currentUserModel = await _usersCollection.getUserModel(currentUserUID!);
      bool sentBySupport = currentUserModel?.role == UserRoles.support || currentUserModel?.role == UserRoles.admin;

      await chatDocRef.set({ // Используем set с merge:true
      ...
      messageController.text = text; // Возвращаем текст, если не удалось отправить
    } finally {
  ...
  Future<void> pickAndUploadImage() async {
    if (_isSending) return;
    final XFile? pickedFile = await _picker.pickImage(source: ImageSource.gallery, imageQuality: 65, maxWidth: 1200);
    if (pickedFile == null) return;
    
    setState(() => _isSending = true);
    File imageFile = File(pickedFile.path);
    String fileName = '${DateTime.now().millisecondsSinceEpoch}_${currentUserUID}_${Uri.file(pickedFile.path).pathSegments.last}';
    Reference storageRef = FirebaseStorage.instance.ref().child('chat_images/${widget.chatId}/$fileName');
    
    try {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Загрузка изображения..."), duration: Duration(seconds: 2)));
  ...
  Widget build(BuildContext context) {
    if (currentUserUID == null) {
      return Scaffold(appBar: AppBar(title: const Text("Ошибка")), body: const Center(child: Text("Ошибка аутентификации.")));
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: const Color.fromARGB(255, 56, 179, 76),
        elevation: 1,
      ),
      body: Column(
        children: [
          Expanded(
            child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                if (!snapshot.hasData || snapshot.data!.docs.isEmpty) return const Center(child: Text('Нет сообщений.'));
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    try {
                       ...
                       print("Error parsing message from Firestore: ${documents[index].id}, error: $e");
     ...
     return Container(
      ...
      child: SafeArea(
        child: Row(
          children: <Widget>[
            IconButton(
              icon: Icon(Icons.add_photo_alternate_outlined, color: Colors.blueGrey.shade600), // Цвет изменен
            ...
            IconButton(
              icon: _isSending 
                  ...
                  : Icon(Icons.send_rounded, color: Colors.blueGrey.shade700), // Цвет изменен
              tooltip: "Отправить",
  ...
  Widget _buildMessageBubble(Message message, bool isMyMessage) {
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isMyMessage ? const Color.fromARGB(255, 48, 122, 26) : const Color.fromARGB(255, 229, 229, 229),
          ...
          children: [
            if (message.imageUrl != null && message.imageUrl!.isNotEmpty)
              Padding(
                ...
                child: ClipRRect(
                  borderRadius: BorderRadius.circular(12), // Скругление для картинки
                  child: Image.network(
                    message.imageUrl!,
                    height: MediaQuery.of(context).size.height * 0.28, // Чуть больше
                    width: MediaQuery.of(context).size.width * 0.7,   // Чуть больше
                    fit: BoxFit.cover,
                    ...
                    errorBuilder: (context, error, stackTrace) => 
                      Container(
                        ...
                        child: Icon(Icons.broken_image_outlined, size: 50, color: Colors.grey.shade500)
                      ),
                  ),
                ),
              ),
            ...
            Text(
              ...
              style: TextStyle(
                color: isMyMessage ? Colors.white.withOpacity(0.85) : Colors.black54.withOpacity(0.85),
                fontSize: 10.5,
              ),
            ),
          ],
